// lib/features/home/pages/offer_ride_page.dart
// NOTE: Adds Google Places autocomplete fields for Origin/Destination/Stops.
// LIVE VEHICLE PHOTO: Mobile & Web picker + Firebase Storage upload + preview + trips/trips_live updates
...
// ------------------------------
// Places Autocomplete (Web)
// ------------------------------

@JS('Object')
external Object _noopObj();
...
class _PlacesWeb {
        ...
        if (google != null) {
          ...
          if (maps != null) {
            ...
            if (acceptLegacy) {
              ...
              final legacyCtor =
                  places == null ? null : js_util.getProperty(places, 'AutocompleteService');
              if (legacyCtor != null) return true;
            }
          }
        }
      } catch (_) {}
  ...
  static Future<List<_PlacePrediction>> autocomplete(
    ...
    try {
      ...
      if (autoSuggestion == null) return _autocompleteLegacy(input, country: country);

      final reqData = <String, dynamic>{'input': input, 'language': 'en'};
      if (country != null) reqData['includedRegionCodes'] = [country];
      final req = js_util.jsify(reqData);

      final fetch = js_util.getProperty(autoSuggestion, 'fetchAutocompleteSuggestions');
      if (fetch == null) return _autocompleteLegacy(input, country: country);

      final resp =
          await js_util.promiseToFuture(js_util.callMethod(fetch, 'call', [autoSuggestion, req]));
      ...
      for (int i = 0; i < length; i++) {
        ...
        final mainText =
            js_util.getProperty(js_util.getProperty(pred, 'mainText') ?? {}, 'text') as String? ??
                '';
      ...
      return _autocompleteLegacy(input, country: country);
    }
  }

  static Future<List<_PlacePrediction>> _autocompleteLegacy(
    String input, {
    String? country,
  }) async {
    try {
      ...
      final reqData = <String, dynamic>{'input': input, 'language': 'en'};
      if (country != null) {
        reqData['componentRestrictions'] = js_util.jsify({'country': country});
      }
...
class _PlacesFieldState extends State<_PlacesField> {
  ...
  void _maybeQuery() {
    ...
    _deb = Timer(const Duration(milliseconds: 300), () async {
      ...
      final res = await _PlacesWeb.autocomplete(q, country: widget.country);
      if (!mounted) return;
      _items = res;
      _updateOverlay();
    });
            ...
            child: ListView(
              ...
              children: [
                ...
                if (!isEditing && _user != null) ...[
                  ...
                  Padding(
                    padding: const EdgeInsets.only(right: 16.0), // Prevent text overflow
                    child: Text(
                      ...
                      return Column(
                        ...
                        children: [
                          DropdownButtonFormField<String>(
                            ...
                            isExpanded: true, // This ensures it takes full width and respects constraints
                            items: docs.map((d) {
                              final data = d.data();
                              final origin =
                ...
                _buildRuleItem(
                 ...
                 subtitle:
                      'Trip fares may be paid online through the app or directly in cash to the driver.',
                ),
                ...
                FormField<bool>(
                  ...
                  validator: (v) => v == true ? null : 'You must agree to the rules to post.',
                  builder: (state) {
                    return Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                            ...
                            Expanded(
                              child: RichText(
                                text: TextSpan(
                                  ...
                                  children: [
                                    const TextSpan(text: 'I agree to these rules, to the '),
                                    TextSpan(
                                      text: 'Driver Cancellation Policy',
                                      style: const TextStyle(
                                        decoration: TextDecoration.underline,
                                        fontWeight: FontWeight.w600,
                                        color: Colors.black, // Make links look clickable
                                      ),
  ...
  Future<void> _uploadVehiclePhoto({
    ...
    try {
      ...
      if (kIsWeb && webBytes != null) {
        task = ref.putData(webBytes, SettableMetadata(contentType: 'image/$ext'));
