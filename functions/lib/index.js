...
exports.autoCompleteTrips = exports.autoStartTrips = exports.updateTripDistanceAndEta = exports.sendSupportTicketEmail = exports.notifyOnChatMessageCreated = exports.notifyRidersOnTripCancelled = exports.notifyRiderOnBookingStatusChange = exports.notifyDriverOnBookingCreated = exports.handleBookingCompletionWallet = exports.handleBookingStatusChange = exports.stripeWebhook = exports.createPaymentIntent = void 0;
const https_1 = require("firebase-functions/v2/https");
const firestore_1 = require("firebase-functions/v2/firestore");
const scheduler_1 = require("firebase-functions/v2/scheduler"); // 🆕 cron
const admin = __importStar(require("firebase-admin"));
const stripe_1 = __importDefault(require("stripe"));
const nodemailer = __importStar(require("nodemailer")); // 🆕 for support email
admin.initializeApp();
...
exports.createPaymentIntent = (0, https_1.onRequest)({ region: 'us-central1', cpu: 1, secrets: ['STRIPE_SECRET'] }, async (req, res) => {
    ...
    try {
        ...
        const amount = Number(body.amount) || 0; // cents
...
exports.stripeWebhook = (0, https_1.onRequest)({ region: 'us-central1', cpu: 1, secrets: ['STRIPE_WEBHOOK_SECRET'] }, async (req, res) => {
    if (req.method !== 'POST') {
    ...
    try {
        switch (event.type) {
            ...
            case 'checkout.session.completed': {
                ...
                console.log('🧾 checkout.session.completed', cs.id, cs.amount_total, cs.currency);
                break;
            }
            default:
...
exports.handleBookingStatusChange = (0, firestore_1.onDocumentUpdated)({
    region: 'us-central1',
    ...
    try {
        console.log(`Creating Stripe refund pi=${paymentIntentId}, amount=${amountCents}`);
...
// ----------------------------------------------------------------------------- 
exports.handleBookingCompletionWallet = (0, firestore_1.onDocumentUpdated)({
    region: 'us-central1',
    document: 'trips/{tripId}/booking_requests/{bookingId}',
}, async (event) => {
    const before = event.data?.before;
    ...
    if (!isFullPayment) {
        console.log('Booking completed but was deposit-only, no wallet credit (platform fee only).');
        return;
    }
    // avoid double-credit
    ...
    try {
        ...
        console.log(`✅ Wallet credited (FULL payment) driver=${driverId}, booking=${bookingId}, net=${driverNetCents} cents, fee=${platformFeeCents} cents`);
    }
    catch (err) {
...
async function sendPushToTokens(tokens, title, body, data = {}) {
    ...
    console.log(`🔔 Sent push "${title}" to ${tokens.length} tokens. success=${response.successCount}, fail=${response.failureCount}`);
}
// -----------------------------------------------------------------------------
// 1) Rider creates booking request → notify DRIVER
// -----------------------------------------------------------------------------
exports.notifyDriverOnBookingCreated = (0, firestore_1.onDocumentCreated)({
    region: 'us-central1',
    document: 'trips/{tripId}/booking_requests/{bookingId}',
}, async (event) => {
    ...
    try {
        ...
        const riderName = (data.riderName ?? 'New rider');
        // ------------------------------
        ...
        await sendPushToTokens(tokens, 'New booking request', `${riderName} requested a seat on your trip ${origin} → ${destination}.`, {
            type: 'booking_request',
            tripId,
            bookingId: event.params.bookingId,
        });
...
// -----------------------------------------------------------------------------
exports.notifyRiderOnBookingStatusChange = (0, firestore_1.onDocumentUpdated)({
    region: 'us-central1',
    document: 'trips/{tripId}/booking_requests/{bookingId}',
}, async (event) => {
    const before = event.data?.before;
...
// -----------------------------------------------------------------------------
exports.notifyRidersOnTripCancelled = (0, firestore_1.onDocumentUpdated)({
    region: 'us-central1',
    ...
    if (newStatus !== 'cancelled' && newStatus !== 'canceled')
        return;
    const tripId = event.params.tripId;
    const origin = (after.get('origin') ?? '');
    const destination = (after.get('destination') ?? '');
    try {
...
// -----------------------------------------------------------------------------
exports.notifyOnChatMessageCreated = (0, firestore_1.onDocumentCreated)({
    region: 'us-central1',
    ...
    try {
        ...
        await sendPushToTokens(tokens, 'New message', shortText, {
            type: 'chat_message',
            chatId: event.params.chatId,
        });
    }
...
// -----------------------------------------------------------------------------
exports.sendSupportTicketEmail = (0, firestore_1.onDocumentCreated)({
    region: 'us-central1',
    ...
    const subject = (data.subject ?? 'New Support Request');
    const details = (data.details ?? '');
    const fromEmail = (data.fromEmail ?? 'unknown@user');
    ...
    if (!mailTransport) {
        console.warn('sendSupportTicketEmail: mailTransport is not configured (missing SUPPORT_SMTP_USER / SUPPORT_SMTP_PASS).');
        await snap.ref.set({
            emailSent: false,
...
// -----------------------------------------------------------------------------
exports.updateTripDistanceAndEta = (0, firestore_1.onDocumentCreated)({
    region: 'us-central1',
    document: 'trips/{tripId}',
    ...
    try {
        ...
        console.log(`✅ updateTripDistanceAndEta: trip=${tripRef.id}, distance=${distanceValue}m, duration=${durationValue}s`);
    }
    catch (err) {
...
// -----------------------------------------------------------------------------
exports.autoStartTrips = (0, scheduler_1.onSchedule)({
    region: 'us-central1',
...
// -----------------------------------------------------------------------------
exports.autoCompleteTrips = (0, scheduler_1.onSchedule)({
    region: 'us-central1',
    schedule: 'every 5 minutes',
}, async () => {
    const nowTs = admin.firestore.Timestamp.now();
    ...
    for (const doc of snap.docs) {
        const data = doc.data();
        const tripId = doc.id;
        const etaTs = data.estimatedArrivalTime;
        const depTs = data.date;
        ...
        console.log(`autoCompleteTrips: trip ${tripId} completed with ${bookingsSnap.size} bookings auto-marked completed`);
    }
});
