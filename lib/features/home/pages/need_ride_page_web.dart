...
class _NeedRidePageState extends State<NeedRidePage> {
  ...
  // ========= Autocomplete (same behaviour as SearchPage) =========
  ...
  // log new->legacy fallback once
  static bool _loggedLegacyOnce = false;

  // ========= Time parsing helper (unchanged) =========
  ...
  Future<void> _fetchPreds({required bool isFrom}) async {
    ...
    try {
      final preds = await _getPlacePredictionsJs(text);
      setState(() {
        if (isFrom) {
          _fromPreds = preds;
  ...
  // New API with graceful fallback to legacy AutocompleteService
  Future<List<_PlacePrediction>> _getPlacePredictionsJs(String input) async {
    try {
      ...
      if (_userLat != null && _userLng != null) {
        // ✅ radius must be between 0 and 50,000 meters
        reqData['locationBias'] = js_util.jsify({
          'center': {'lat': _userLat, 'lng': _userLng},
          'radius': 40000, // was 150000 -> fixed to 40km
        });
      }
      ...
      for (int i = 0; i < length; i++) {
        ...
        final mainText =
            js_util.getProperty(js_util.getProperty(pred, 'mainText') ?? {}, 'text')
                    as String? ??
                '';
        final secondaryText =
            js_util.getProperty(js_util.getProperty(pred, 'secondaryText') ?? {}, 'text')
                    as String? ??
                '';
        final placeId = js_util.getProperty(pred, 'placeId') as String? ?? '';

        final description =
            [mainText, if (secondaryText.isNotEmpty) secondaryText].join(', ');
      ...
      if (msg.contains('PERMISSION_DENIED') ||
        ...
        if (!_loggedLegacyOnce) {
          ...
          print('! New Places API blocked — falling back to legacy AutocompleteService.');
        }
        return _getPlacePredictionsLegacy(input);
      }
  ...
  Future<List<_PlacePrediction>> _getPlacePredictionsLegacy(String input) async {
    try {
      ...
      final reqData = <String, dynamic>{
        'input': input,
        'language': 'en',
        // allow POIs and localities for richer results
        // 'types': ['(cities)'],
      };
      ...
      if (_userLat != null && _userLng != null) {
        ...
        if (latLngCtor != null) {
          ...
          reqData['radius'] = 40000; // ✅ keep under 50,000m
        }
      }
  ...
  // ================= Overlays (same interaction fixes) =================
  ...
  Future<void> _pickDate() async {
    ...
    final init =
        _dateCtrl.text.isEmpty ? now.add(const Duration(days: 1)) : (DateTime.tryParse(_dateCtrl.text) ?? now);
  ...
  void _showTimeDropdown(BuildContext context) async {
    final now = DateTime.now();
    TimeOfDay initialTime = _timeOfDayFromController ?? TimeOfDay.fromDateTime(now);

    final snappedMinute = (initialTime.minute / 15).round() * 15;
    initialTime =
        initialTime.replacing(minute: snappedMinute % 60, hour: initialTime.hour + (snappedMinute ~/ 60));
    ...
    if (picked != null) {
      ...
      final finalTime = picked.replacing(minute: minute % 60, hour: picked.hour + (minute ~/ 60));

      final dt = DateTime(now.year, now.month, now.day, finalTime.hour, finalTime.minute);
      setState(() {
  ...
  // ---------- submit (unchanged, except text fields now filled via picker too) ----------
  Future<void> _submit() async {
    ...
    if (!isEditing) {
      if (dep == null || !dep.isAfter(DateTime.now())) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('The selected date and time must be in the future.')),
        );
        return;
      }
    } else {
        ...
        child: Form(
          ...
          child: ListView(
            ...
            children: [
              ...
              // ⤵️ same anchored target + overlay tap-to-select
